...
export default function InformacionDePago() {
    const { informacion } = usePage().props;

    const [text, setText] = useState(informacion?.informacion || '');
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        post(route('admin.informacion-de-pago.update'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    useEffect(() => {
        setData('informacion', text);
    }, [text]);

    return (
        <Dashboard>
            <form onSubmit={handleUpdate} className="flex h-fit flex-col justify-between gap-5 p-6">
                <h2 className="border-primary-orange text-primary-orange text-bold w-full border-b-2 text-2xl">Informacion de pago</h2>
