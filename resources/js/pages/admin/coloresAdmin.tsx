import ColorAdminRow from '@/components/colorAdminRow';
...
export default function ColoresAdmin() {
    ...
    const handleSubmit = (e) => {
        ...
        post(route('admin.colores.store'), {
            preserveScroll: true,
            onSuccess: () => {
        ...
        <Dashboard>
            <div className="flex w-full flex-col p-6">
                        ...
                        >
                            <form onSubmit={handleSubmit} method="POST" className="text-black">
                                <div className="w-[500px] rounded-md bg-white p-4">
                                    <h2 className="mb-4 text-2xl font-semibold">Crear Color</h2>
                                    <div className="flex flex-col gap-4">
                                        <div className="flex w-full flex-row gap-5">
                                            <div className="flex w-full flex-col gap-3">
                                                <label htmlFor="nombree_en">Color</label>
                                                <input
                                                    className="focus:outline-primary-orange h-full rounded-md p-2 outline outline-gray-300 focus:outline"
                                                    type="color"
                                                    name="nombree_en"
                                                    id="nombree_en"
                ...
                <div className="mx-auto flex w-full flex-col gap-3">
                    <h2 className="border-primary-orange text-primary-orange text-bold w-full border-b-2 text-2xl">Colores</h2>
                        ...
                        <table className="w-full border text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                            <thead className="bg-gray-300 text-sm font-medium text-black uppercase">
                                <tr>
                                    ...
                                    <td className="py-2 text-center">COLOR</td>
                                    <td className="text-center">EDITAR</td>
                                </tr>
                            </thead>
                            <tbody className="text-center">{colores.data?.map((color) => <ColorAdminRow key={color.id} color={color} />)}</tbody>
                        </table>
                    </div>
                            ...
                            <div className="flex items-center">
                                {colores.links.map((link, index) => (
                                    <button
                                        ...
                                        } ${index === 0 ? 'rounded-l-md' : ''} ${index === colores.links.length - 1 ? 'rounded-r-md' : ''}`}
                    ...
                    <div className="mt-2 text-center text-sm text-gray-600">
                        Mostrando {colores.from || 0} a {colores.to || 0} de {colores.total} resultados
                    </div>
                </div>
            </div>
        </Dashboard>
