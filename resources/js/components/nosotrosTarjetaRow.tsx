...
export default function NosotrosTarjetaRow({ tarjeta }) {
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.nosotros-tarjetas.update'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const deleteTarjeta = () => {
        if (confirm('¿Estas seguro de eliminar esta tarjeta?')) {
            updateForm.delete(route('admin.nosotros-tarjetas.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
