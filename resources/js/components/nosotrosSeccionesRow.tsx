...
export default function NosotrosSeccionesRow({ seccion }) {
    const [edit, setEdit] = useState(false);

    const updateForm = useForm({
        order: seccion?.order,
        name_es: seccion?.name_es,
        name_en: seccion?.name_en,
        text_es: seccion?.text_es,
        text_en: seccion?.text_en,
        id: seccion?.id,
    });

    const [textEs, setTextEs] = useState(seccion?.text_es);
    const [textEn, setTextEn] = useState(seccion?.text_en);
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.nosotros-secciones.update'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const deleteSeccion = () => {
        if (confirm('¿Estas seguro de eliminar esta sección?')) {
            updateForm.delete(route('admin.nosotros-secciones.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <tr className={`border text-black odd:bg-gray-100 even:bg-white`}>
            <td className="align-middle">{seccion?.order}</td>
            <td className="align-middle">{seccion?.name_es}</td>
            <td className="align-middle">{seccion?.name_en}</td>
            <td className="h-[90px] align-middle">
                <div className="line-clamp-3 max-w-[200px] overflow-hidden break-words" dangerouslySetInnerHTML={{ __html: seccion?.text_es }} />
            </td>
            <td className="h-[90px] align-middle">
                <div className="line-clamp-3 max-w-[200px] overflow-hidden break-words" dangerouslySetInnerHTML={{ __html: seccion?.text_en }} />
            </td>
            <td className="h-[90px] align-middle">
                <img src={seccion?.image} className="h-full w-full" alt="" />
            </td>
            ...
            <td className="w-[140px] text-center">
                <div className="flex flex-row justify-center gap-3">
                    ...
                    <button onClick={deleteSeccion} className="h-10 w-10 rounded-md border border-red-500 px-2 py-1 text-white">
